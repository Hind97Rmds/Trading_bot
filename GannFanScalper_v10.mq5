...
input bool     InpRebaseTpSlOnFill    = false;      // إعادة محاذاة TP/SL على سعر التنفيذ الفعلي بعد الفتح
                                                      // (لا/false = مطابقة اقتصاديات باكتيست بايثون تماماً:
                                                      // TP/SL يبقيان مرتكزَين على المستوى النظري دائماً، فلا
                                                      // يتحول هامش اللمس الواسع لانزلاق حقيقي في النتائج.
                                                      // نعم/true = واقعية أكبر للتداول الحي: يُعاد ضبط TP/SL
                                                      // على السعر الحقيقي الذي نُفِّذت به الصفقة فعلاً، فتبقى
                                                      // نسبة المخاطرة/العائد صحيحة مهما بلغ الانزلاق -- لكن هذا
                                                      // يجعل نتائج التداول الحي تختلف عن نتائج باكتيست بايثون
                                                      // كلما اتسع هامش اللمس.
...
// ── Anchor-cycle history (drives the new "دورات الأنكر ومستوياتها" sheet) ──
// One entry per new Gann cycle: the anchor close price/time, plus the FULL
// resulting level ladder for that cycle (serialized so each cycle can list
// all of its own levels underneath it, independent of what the CURRENT
// live g_levels[] array holds afterwards).
datetime    c_anchorTime[];
double      c_anchorClose[];
string      c_levelsSerialized[];   // "key1:price1:⭐/🌀/•|key2:price2:...|..."
...
string          g_tfName[];
datetime        g_lastCloseCheckBar[];  // per-tf: bar-open-time of the last bar whose
                                        // "close" channel was already evaluated -- gates
                                        // that channel to fire at most ONCE per newly-
                                        // closed bar, matching Python's once-per-closed-
                                        // candle cadence instead of re-checking on every
                                        // single tick within the same still-open bar.
...
void AddTF(bool enabled, ENUM_TIMEFRAMES tf, string name)
{
   ...
   ArrayResize(g_lastCloseCheckBar, n+1);
   g_tfList[n] = tf;
   g_tfName[n] = name;
   g_lastCloseCheckBar[n] = 0;
}
...
void BuildEnabledTFs()
{
   ...
   ArrayResize(g_lastCloseCheckBar,0);
...
{
   int existing = RegistryFind(posid);
   if(existing >= 0)
   {
      // Same position id already tracked (this happens on a NETTING account:
      // MT5 merges every new same-symbol trade into the ONE existing net
      // position instead of creating an independent one -- see the OnInit
      // warning). Overwrite in place instead of appending a stale duplicate,
      // so RegistryFind() always returns the latest true state of what the
      // broker is actually holding.
      g_posTF[existing]=tf; g_posLevel[existing]=level; g_posEntry[existing]=entry;
      g_posTP[existing]=tp; g_posSL[existing]=sl; g_posTrigger[existing]=trigger;
      g_posIsBuy[existing]=isBuy; g_posReal[existing]=isReal; g_posOpen[existing]=openT;
      g_posBE[existing]=false;
      return;
   }
...
void RecordCycleHistory(datetime anchorTime, double anchorClose)
{
   string serialized = "";
   for(int i=0; i<ArraySize(g_levels); i++)
   {
      SGannLevel lv = g_levels[i];
      if(lv.dir == 2) continue; // skip the "ref" pseudo-level, not tradeable
      string tag = lv.star ? "⭐" : (lv.fan ? "🌀" : "•");
      string dirLbl = (lv.dir == 0) ? "مقاومة/بيع" : "دعم/شراء";
      if(serialized != "") serialized += "|";
      serialized += StringFormat("%s %s %s (%s)", tag, lv.key, DoubleToString(lv.price,_Digits), dirLbl);
   }
   int n = ArraySize(c_anchorTime);
   ArrayResize(c_anchorTime, n+1);
   ArrayResize(c_anchorClose, n+1);
   ArrayResize(c_levelsSerialized, n+1);
   c_anchorTime[n]        = anchorTime;
   c_anchorClose[n]       = anchorClose;
   c_levelsSerialized[n]  = serialized;
}
...
void ManageCycle()
{
   datetime anchorBarTime = iTime(_Symbol, InpAnchorTF, 1); // OPEN time of the last CLOSED anchor bar
   if(anchorBarTime <= 0) return;
   // The bar's actual CLOSE time (what should be shown/logged as "إغلاق
   // الأنكر") is its open time + one full bar period -- NOT anchorBarTime
   // itself. Using anchorBarTime directly as a "close time" label made every
   // cycle in this EA's reports appear exactly 1 hour earlier than the same
   // cycle in the Telegram/Python backtest's reports (confirmed by matching
   // anchor CLOSE PRICES between the two side by side). The rollover
   // comparisons below intentionally keep using the raw open-time
   // (anchorBarTime/g_lastAnchorTime) since that's self-consistent for
   // detecting a NEW vs SAME bar either way -- only the value shown to the
   // user needs the +1 bar-period correction.
   datetime anchorCloseTime = anchorBarTime + PeriodSeconds(InpAnchorTF);
   ...
   g_cycleActive    = true;
   ClearUsed();
   RecordCycleHistory(anchorCloseTime, h1close);

   Journal(StringFormat("🔄 دورة جان جديدة (%dس) | إغلاق %s الساعة %s: %s | عدد المستويات النشطة: %d",
           InpCycleHours, EnumToString(InpAnchorTF), DamString(anchorCloseTime),
...
double price = mid;
    ...
    // IMPORTANT: TP/SL are anchored to the THEORETICAL LEVEL PRICE (lv.price),
    // not to the actual live fill price (`price`/mid). This matches Python's
    // zero-margin backtest assumption (entry = lv['price'] always).
    // Python's zero-margin architecture means touch is ONLY confirmed when the
    // exact level price falls within the candle's physical range (Low <= level <= High).
    // There is no detection tolerance -- the level must be physically touched.
    double tp = isBuy ? PriceRound(lv.price + tpDist) : PriceRound(lv.price - tpDist);
    double sl = isBuy ? PriceRound(lv.price - slDist) : PriceRound(lv.price + slDist);

    if((isBuy && (price >= tp || price <= sl)) || (!isBuy && (price <= tp || price >= sl)))
    {
       MarkUsed(comboKey);
       Journal(StringFormat("⏭️ [%s %s] أُلغيت قبل الإرسال: السعر الفعلي (%s) تجاوز TP/SL المرتكز على "
                "المستوى (TP:%s SL:%s).",
                _Symbol, tfName, DoubleToString(price,_Digits), DoubleToString(tp,_Digits),
...
// rebase TP/SL onto the ACTUAL fill -- OFF by default now (see
    // InpRebaseTpSlOnFill above): keeping TP/SL anchored to the level (as
    // set above) is what makes zero-margin detection behave identically to
    // Python's backtest (strict touch detection only). Turn this ON only if
    // you specifically want live-trading risk/reward realism instead of
    // backtest-parity.
   if(InpRebaseTpSlOnFill && MathAbs(fill - price) > _Point/2.0)
   {
...
void EntryScan()
{
   ...
   // trend: 1=up, -1=down, 0=undecided (EMA/VWAP not ready yet, or -- in
   // TREND_BOTH mode -- EMA and VWAP disagree). Python's equivalent
   // (macro_trend_up) treats "undecided" as "no filter, every level stays
   // tradeable" -- NOT as "block everything." The old code below used to
   // `continue` (skip the whole timeframe) whenever trend==0, which is the
   // opposite of Python's behavior and silently lost every trade during
   // EMA warm-up or any ambiguous-trend moment. Fixed: only apply the
   // directional filter per level when trend is actually decisive.
...
double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
    ...
    for(int t=0; t<ArraySize(g_tfList); t++)
    {
       ...
       double closedHigh  = iHigh(_Symbol, tf, 1);
       double closedLow   = iLow(_Symbol, tf, 1);
       if(closedClose <= 0 || closedHigh <= 0 || closedLow <= 0) continue;
       datetime closedBarTime = iTime(_Symbol, tf, 1);
       bool allowCloseChannel = (closedBarTime != g_lastCloseCheckBar[t]);
       if(allowCloseChannel) g_lastCloseCheckBar[t] = closedBarTime;

       for(int ch=0; ch<ArraySize(channels); ch++)
       {
          string channel = channels[ch];
          if(channel == "close" && !allowCloseChannel) continue;
          ...
          for(int i=0; i<ArraySize(g_levels); i++)
          {
             ...
             if(InpEntryMode == ENTRY_TOUCH_TREND && trend != 0)
             {
             ...
             bool touched = false;
             if(channel == "close")
                touched = (closedClose == lv.price);
             else if(channel == "hybrid")
             {
                if(!(closedLow <= lv.price && lv.price <= closedHigh)) touched = false;
                else if(InpSpikeFilter && MathAbs((bid+ask)/2.0 - closedClose) > spike) touched = false;
                else touched = true;
             }
             else // touch
                touched = (closedLow <= lv.price && lv.price <= closedHigh);

             if(!touched) continue;

             Journal(StringFormat("🎯 لمس مستوى [%s] %s على فريم %s (قناة %s) | السعر: %s | المستوى: %s",
                     (isBuy?"دعم/شراء":"مقاومة/بيع"), _Symbol, tfName, channel,
                     DoubleToString((bid+ask)/2.0,_Digits), DoubleToString(lv.price,_Digits)));
...
void WriteExcelReport()
{
   ...
   xml += "</Table></Worksheet>" + nl;

   // ── second sheet: anchor cycles + their resulting Gann levels ──
   xml += "<Worksheet ss:Name=\"دورات الأنكر ومستوياتها\"><Table>" + nl;
   string chdrs[] = {"رقم الدورة","وقت إغلاق الأنكر (DAM)","سعر إغلاق الأنكر","المستوى","السعر","النوع/الاتجاه"};
   xml += "<Row>";
   for(int i=0; i<ArraySize(chdrs); i++)
      xml += StrCell("hdr", chdrs[i]);
   xml += "</Row>" + nl;

   for(int c=0; c<ArraySize(c_anchorTime); c++)
   {
      // one header-style row per cycle stating its close time/price ...
      xml += "<Row>";
      xml += NumCell("pln", c+1, 0);
      xml += StrCell("pln", DamString(c_anchorTime[c]));
      xml += NumCell("pln", c_anchorClose[c], _Digits);
      xml += StrCell("pln", "— (إغلاق الأنكر لهذه الدورة) —");
      xml += "<Cell></Cell><Cell></Cell>";
      xml += "</Row>" + nl;

      // ... then every resulting level for that cycle listed directly below it
      string parts[];
      int cnt = StringSplit(c_levelsSerialized[c], StringGetCharacter("|",0), parts);
      for(int p=0; p<cnt; p++)
      {
         string sub[];
         int scnt = StringSplit(parts[p], StringGetCharacter(" ",0), sub);
         // parts[p] format: "TAG key price (dirLbl)" -- re-split on the fixed
         // separators used when it was built, to place price in its own
         // numeric cell instead of dumping the raw string.
         string tag = (scnt>0) ? sub[0] : "";
         string key = (scnt>1) ? sub[1] : "";
         double px  = (scnt>2) ? StringToDouble(sub[2]) : 0.0;
         string dirLbl = "";
         for(int k=3;k<scnt;k++) dirLbl += sub[k] + " ";

         xml += "<Row>";
         xml += "<Cell></Cell><Cell></Cell><Cell></Cell>";
         xml += StrCell("pln", tag+" "+key);
         xml += NumCell("pln", px, _Digits);
         xml += StrCell("pln", dirLbl);
         xml += "</Row>" + nl;
      }
      xml += "<Row></Row>" + nl; // blank spacer row between cycles
   }
   xml += "</Table></Worksheet>" + nl;
...
int OnInit()
{
   ...
   {
      Journal("🚨 تحذير هام: هذا الحساب ليس Hedging! هذا الإكسبيرت يفترض أن كل توليفة"
              " (مستوى + فريم) تفتح صفقة مستقلة بـTP/SL خاص بها. على حساب Netting،"
              " يدمج MT5 كل الصفقات على نفس الرمز في صفقة صافية واحدة بمتوسط سعر دخول"
              " وTP/SL واحد فقط — فتتجاوز كل صفقة جديدة إعدادات التي قبلها بصمت، وتختلف"
              " نتائج تقرير Excel هذا عن تقرير MT5 نفسه. بدّل نوع الحساب إلى Hedging"
              " (Strategy Tester أو حساب تجريبي بديل) قبل الاعتماد على هذا التقرير.");
   }
