"""
telegram_ui.py — Telegram Bot interface for the Gann backtest bot.

Owns:
  - Keyboard builders (main, protection, gann, backtest, presets)
  - Message sending (send_tg_msg, edit_tg_msg, send_tg_document)
  - Callback handler (_handle_callback)
  - Telegram polling loop & watchdog
  - /set, /backtest, /csvdir, /csvoffset command parsing
"""
...
from state import (
    bot_state, _TFS, AVAILABLE_SYMBOLS, TG_TOKEN,
    PRESETS_FILE, _PRESET_EXCLUDED_KEYS,
    get_http, log_exception, c_log, _safe_task, _debounced_persist_save,
    save_bot_persistence, _write_presets_file_sync, DAM_OFF,
)
from strategy import (
    _anchor_label, _anchor_hours, gann_calc_levels,
...
async def _show(chat_id, msg_id, text: str, reply_markup: dict = None) -> None:
    if msg_id:
        await edit_tg_msg(chat_id, msg_id, text, reply_markup)
    else:
        await send_tg_msg(text, reply_markup)
...
def get_main_keyboard() -> dict:
    return {'inline_keyboard': [
        ...
        [{'text': '🛡️ إعدادات الحماية (لباكتيست)', 'callback_data': 'menu_protection'}],
...
def get_protection_keyboard() -> dict:
    ...
    rows = [
        [{'text': '── الحدود اليومية (باكتيست) ──', 'callback_data': 'noop'}],
        ...
        [{'text': '── فلاتر الباكتيست ──', 'callback_data': 'noop'}],
        [{'text': f"إلغاء الدورة وقت الانفجار: {'✅' if bot_state.get('prot_cycle_inval', True) else '🔴'}",
          'callback_data': 'tg_prot_inval'}],
        [{'text': f"فلتر انفجار السعر (Hybrid): {'✅' if bot_state.get('prot_spike_filter', True) else '🔴'}",
          'callback_data': 'tg_prot_spike'}],
        [{'text': f"حد الانفجار: {bot_state.get('gann_spike_limit_pts', 20)} نقطة", 'callback_data': 'noop'}],
        [{'text': '➖ 5 نقاط', 'callback_data': 'gann_dec_spike'},
         {'text': '➕ 5 نقاط', 'callback_data': 'gann_inc_spike'}],
        [{'text': f"BE شامل التكلفة: {'✅' if bot_state.get('prot_cost_be', True) else '🔴'}",
          'callback_data': 'tg_prot_cost'}],
        [{'text': f"إطار مرجعي للجان: {bot_state.get('gann_anchor_tf', '1h').upper()}",
          'callback_data': 'tg_prot_anchor'}],
        [{'text': f"فلتر أوقات دمشق: {'✅' if bot_state.get('prot_dam_time_filter', True) else '🔴'}",
          'callback_data': 'tg_prot_dam_time'}],
        [{'text': f"حساب جان: {'⚡ ديناميكي' if bot_state.get('gann_calculation_mode', 'static_h1') == 'dynamic_live' else '📌 كلاسيكي'}",
          'callback_data': 'tg_gann_calc_mode'}],
...
def get_gann_keyboard() -> dict:
    ...
    if zf == 'star':
        zf_lbl = '⭐ المستويات الأصلية القوية فقط'
    elif zf == 'star_fan':
        zf_lbl = '⭐🌀 القوية + موازية للمروحة'
    else:
        zf_lbl = '📋 كل المستويات (للتجارب)'
    if flt_type == 'ema':
    ...
    em_lbl = (
        f'⚡ لمس + فلتر ({flt_name}_{ttf_lbl})'
        if em == 'touch_trend' else '⚡ لمس أعمى (بدون فلتر)'
    )
    tps_lbl = f'🎯 TP/SL: {"نقاط ثابتة" if tpsm == "fixed" else "حسب ATR"}'
    tp = sym_state['gann_tp_points']
    sl = sym_state['gann_sl_points']
    atp = sym_state['gann_atr_tp_mult']
    asp = sym_state['gann_atr_sl_mult']
    ...
    exec_lbl = {
        ...
        'all_concurrent': '🔀 الثلاثة معاً (All-Concurrent)',
    }.get(exec_mode, '⚡ دخول لمس مباشر (Instant)')
    rows = [
        [{'text': f'📐 {sym}', 'callback_data': 'noop'}],
        [{'text': '🔄 عرض الدعوم والمقاومات (آخر إغلاق)', 'callback_data': 'gann_show_levels'}],
    ...
    if flt_type == 'vwap':
        ...
        rows.append([
            {'text': 'VWAP −10', 'callback_data': 'gann_dec_vwap'},
    ...
    if flt_type == 'ema':
        ...
        rows.append([
            {'text': 'EMA −10', 'callback_data': 'gann_dec_ema'},
    ...
    for i in range(0, len(tf_items), 4):
        rows.append([
            {'text': ('✅' if on else '⬜') + f' {tfk}', 'callback_data': f'gann_tf_{tfk}'}
            for tfk, on in tf_items[i:i + 4]
        ])
    rows += [
...
def get_gann_tpsl_tf_keyboard(sel_tf: str = '') -> dict:
    ...
    rows = [
        [{'text': '⚙️ TP/SL مخصص لكل فريم', 'callback_data': 'noop'}],
    ...
    if tf_row:
        rows.append(tf_row)
    if sel_tf:
        tp_v = sym_state['gann_tp_per_tf'].get(sel_tf, 0)
        sl_v = sym_state['gann_sl_per_tf'].get(sel_tf, 0)
...
def get_gann_bt_keyboard() -> dict:
    if bot_state['is_backtesting']:
        return {'inline_keyboard': [
            [{'text': '⏳ الباكتيست يعمل...', 'callback_data': 'noop'}],
...
async def _cb_menu_main(chat_id, msg_id, sym, sym_state):
    await _show(chat_id, msg_id, '<b>مرحباً بك في Gold Scalper Backtest Bot</b>', get_main_keyboard())
...
async def _cb_menu_presets(chat_id, msg_id, sym, sym_state):
    await _show(chat_id, msg_id, '<b>إدارة الإعدادات (Presets):</b>', {
        'inline_keyboard': [
            [{'text': '💾 حفظ كـ Preset 1', 'callback_data': 'save_preset_1'},
             {'text': '📂 تحميل Preset 1', 'callback_data': 'load_preset_1'}],
            [{'text': '💾 حفظ كـ Preset 2', 'callback_data': 'save_preset_2'},
             {'text': '📂 تحميل Preset 2', 'callback_data': 'load_preset_2'}],
            [{'text': '💾 حفظ كـ Preset 3', 'callback_data': 'save_preset_3'},
             {'text': '📂 تحميل Preset 3', 'callback_data': 'load_preset_3'}],
            [{'text': '🔙 رجوع', 'callback_data': 'menu_main'}],
        ]
    })
...
async def _cb_menu_protection(chat_id, msg_id, sym, sym_state):
    await _show(chat_id, msg_id, 'إعدادات الحماية (تؤثر على الباكتيست):', get_protection_keyboard())


@_exact('menu_gann')
async def _cb_menu_gann(chat_id, msg_id, sym, sym_state):
    await _show(chat_id, msg_id, 'إعدادات جان (لباكتيست):', get_gann_keyboard())
...
async def _cb_tg_gann_calc_mode(chat_id, msg_id, sym, sym_state):
    new_mode = (
        'static_h1'
        if bot_state.get('gann_calculation_mode', 'static_h1') == 'dynamic_live'
        else 'dynamic_live'
    )
    bot_state['gann_calculation_mode'] = new_mode
    await save_bot_persistence()
    await _show(chat_id, msg_id, '🛡️ إعدادات الحماية:', get_protection_keyboard())
    mode_lbl = '⚡ ديناميكي' if new_mode == 'dynamic_live' else '📌 كلاسيكي (Static)'
...
async def _cb_gann_show_levels(chat_id, msg_id, sym, sym_state):
    ...
    if last_h1:
        ...
        await send_tg_msg(_gann_fmt_levels_msg(sym, h1_close))
    else:
        await send_tg_msg('❌ تعذّر جلب البيانات.')
...
async def _cb_gann_toggle_entry(chat_id, msg_id, sym, sym_state):
    sym_state['gann_entry_mode'] = (
        'pure_touch' if sym_state['gann_entry_mode'] == 'touch_trend' else 'touch_trend'
    )
    await _show(chat_id, msg_id, 'إعدادات جان:', get_gann_keyboard())
...
async def _cb_gann_toggle_filter(chat_id, msg_id, sym, sym_state):
    ...
    sym_state['gann_zone_filter'] = (
        'star_fan' if cur == 'star' else 'all' if cur == 'star_fan' else 'star'
    )
    await _show(chat_id, msg_id, 'إعدادات جان:', get_gann_keyboard())
...
async def _pfx_save_preset(d, chat_id, msg_id, sym, sym_state):
    ...
    if os.path.exists(PRESETS_FILE):
        try:
            ...
            log_exception(f"save_preset_{p_num} (reading)", e)
            data = {}
    import json
...
async def _pfx_load_preset(d, chat_id, msg_id, sym, sym_state):
    ...
    if not os.path.exists(PRESETS_FILE):
        await send_tg_msg("❌ لا يوجد ملف Presets محفوظ بعد.")
        return
    try:
        import json
        data = json.load(open(PRESETS_FILE))
...
def _make_tptf_handler(prefix, attr, delta):
    async def _h(d, chat_id, msg_id, sym, sym_state):
        tf = d[len(prefix):]
        if delta < 0:
            sym_state[attr][tf] = max(0, sym_state[attr].get(tf, 0) + delta)
        else:
            sym_state[attr][tf] = sym_state[attr].get(tf, 0) + delta
...
async def _pfx_tptf_rst(d, chat_id, msg_id, sym, sym_state):
    ...
    sym_state['gann_tp_per_tf'][tf] = 0
    sym_state['gann_sl_per_tf'][tf] = 0
...
async def telegram_polling_loop() -> None:
    ...
    try:
        while True:
            try:
                async with sess.get(
                    url, params={'offset': bot_state['last_update_id'] + 1, 'timeout': 20}
                ) as resp:
                    if resp.status == 200:
                        backoff = 1
...
async def process_tg_update(update: dict) -> None:
    if 'message' in update and 'text' in update['message']:
        ...
        if parts[0] == '/set':
            ...
            if len(parts) == 3 and parts[1] in ['ema', 'vwap'] and parts[2].isdigit():
                val = int(parts[2])
                if parts[1] == 'ema':
                    sym_state['trend_ema_period'] = val
                elif parts[1] == 'vwap':
                    sym_state['trend_vwap_period'] = val
                await save_bot_persistence()
                ...
                if tf in _TFS and param in ['tp', 'sl'] and val.isdigit():
                    val = int(val)
                    if param == 'tp':
                        sym_state['gann_tp_per_tf'][tf] = val
                    elif param == 'sl':
                        sym_state['gann_sl_per_tf'][tf] = val
                    await save_bot_persistence()
                    await send_tg_msg(
                        f"✅ <b>تم التحديث!</b>\n📌 الفريم: {tf}\n⚙️ {param.upper()}: {val}"
                    )
        ...
        if msg.startswith('/csvdir'):
            ...
            if len(parts_csv) == 1 or parts_csv[1].strip().lower() in ('off', 'none', 'clear'):
                ...
                await save_bot_persistence()
                await send_tg_msg(
                    "✅ تم إيقاف مصدر CSV — الباكتيست سيعود لجلب البيانات من OANDA كالمعتاد."
                )
                return
            path = parts_csv[1].strip()
            if not os.path.isdir(path):
                await send_tg_msg(
                    f"❌ المسار غير موجود على السيرفر: <code>{path}</code>\n"
            ...
            await save_bot_persistence()
            await send_tg_msg(
                f"✅ سيقرأ الباكتيست الآن من ملفات CSV في: <code>{path}</code>\n"
                f"(إن لم يجد ملف الرمز/الفريم المطلوب هناك، يعود تلقائياً لـOANDA).\n"
                f"أوفست توقيت خادم الوسيط الحالي: "
                f"{bot_state.get('csv_broker_utc_offset_hours', 3.0)} ساعة "
        ...
        if msg.startswith('/csvoffset'):
            ...
            try:
                ...
                await save_bot_persistence()
                await send_tg_msg(
                    f"✅ أوفست توقيت خادم الوسيط الآن: "
                    f"{bot_state['csv_broker_utc_offset_hours']} ساعة"
                )
            except ValueError:
        ...
        if not msg.startswith('/') and msg in bot_state.get('menu_button_map', {}):
            ...
            if cb != 'noop':
                await _handle_callback(cb, bot_state['chat_id'], None)
            return
        if msg == '/start':
            await send_tg_msg(
                '<b>مرحباً بك في Gold Scalper Backtest Bot</b>\n'
                'محرك الباكتيست فقط — بدون تداول حي.',
                get_main_keyboard(),
            )
        else:
            await send_tg_msg("❌ أمر غير معروف. استخدم /start لعرض القائمة.")
        return
    ...
    try:
        ...
        log_exception(f'callback dispatch [{d}]', e)
