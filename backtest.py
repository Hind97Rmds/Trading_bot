            ...
            for cdef in cycle_defs:
                ...
                for btf, candles_m in monitor_tfs_data.items():
                    ...
                    for bar in m_window:
                        ...
                        trend_up = True
                        if sym_state['gann_entry_mode'] == 'touch_trend':
                            # CAUSALITY FIX (no lookahead bias): see identical fix/comment in
                            # the Gann-cycle backtest loop above. We must reference the last
                            # fully-closed trend candle, not the one currently forming under
                            # bar_time, otherwise the filter "knows" the hour's outcome early.
                            trend_time = bar_time.floor(trend_freq) - pd.Timedelta(trend_freq)
                            if trend_time in df_trend.index:
                                val = df_trend.loc[trend_time, 'macro_trend_up']
                                if isinstance(val, pd.Series): val = val.iloc[-1]
